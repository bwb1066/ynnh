 ...
 * @returns {string} - 'youtube', 'vimeo', or 'aem-video'
 */
 ...
 * @param {string} source - The video source type ('youtube', 'vimeo', or 'aem-video')
...
function getVideoTypeLabel(source) {
  ...
  return labels[source] || 'aem-video';
}
...
function getVideoElement(source, autoplay, background) {
  ...
  sourceEl.setAttribute('type', `aem-video/${source.split('.').pop()}`);
...
export default async function decorate(block) {
  ...
  if (placeholder) {
    ...
    wrapper.className = 'aem-video-placeholder';
    ...
    if (!autoplay) {
      ...
      wrapper.insertAdjacentHTML(
        'beforeend',
        `<div class="aem-video-placeholder-play"><button type="button" title="${ariaLabel}" aria-label="${ariaLabel}"></button></div>`,
      );
