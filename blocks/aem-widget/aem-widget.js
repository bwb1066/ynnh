...
function widgetUrl(widgetPath, widgetName, extension) {
  ...
  return `${getConfig().codeBase}/widgets/${prefix}${widgetName}.${extension}`;
}
...
export default async function decorate(widget) {
  ...
  try {
    ...
    const cssLoaded = loadStyle(widgetUrl(widgetPath, widgetName, 'css'));
    ...
    const wrapper = widget.closest('.aem-widget-wrapper');
    if (wrapper) {
      wrapper.classList.add(`${widgetName}-wrapper`);
      wrapper.classList.remove('aem-widget-wrapper');
    }
    const container = widget.closest('.aem-widget-container');
    if (container) {
      container.classList.add(`${widgetName}-container`);
      container.classList.remove('aem-widget-container');
    }
  } catch (error) {
